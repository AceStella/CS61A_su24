...
def mario_number(level):
    ...
    if len(level) < 3:
        return 1
    if level[1] == "P": #need to jump right now
        if level[2] == "P": #cannot jump two plants
            return 0
        #jump two spots, 1 way
        return mario_number(level[2:])
    else: 
        if level[2] == "P": #cannot jump
...
def max_subseq(n, t):
    ...
    exclude_last = max_subseq(rest_of_digits, t)
    
    # Include the last digit
    include_last = max_subseq(rest_of_digits, t - 1) * 10 + last_digit
    
    return max(exclude_last, include_last)
...
def is_prime(n):
    ...
    def verify_factor(i):
        if i == n:
            ...
            return verify_factor(i+1)
    return verify_factor(2)
