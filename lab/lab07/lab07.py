...
def midsem_survey(p):
    ...
    """
    import hashlib
    return f'c20d4e5854c4c9cdfd14626aad39bd5a1e2ed9bb30dca56d5643a3ad'
...
class BankAccount:
    ...
    def __init__(self, account_holder):
        ...
        self.holder = account_holder
        self.transactions = []
        self.last_transaction = Transaction(-1, 0, 0)

    def deposit(self, amount):
        """Increase the account balance by amount, add the deposit
        to the transaction history, and return the new balance.
        """
        self.last_transaction = Transaction(len(self.transactions), self.balance, self.balance + amount)
        self.balance = self.balance + amount
    ...
    def withdraw(self, amount):
        ...
        """
        if amount > self.balance:
            self.last_transaction = Transaction(len(self.transactions), self.balance, self.balance)
            self.transactions.append(self.last_transaction)
            return 'Insufficient funds'
        self.last_transaction = Transaction(len(self.transactions), self.balance, self.balance - amount)
        self.balance = self.balance - amount
