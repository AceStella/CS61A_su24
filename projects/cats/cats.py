...
def pick(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    good = [word for word in paragraphs if select(word)]
    if k >= len(good):
        return ''
    else:
        return good[k]
...
def about(subject):
    ...
    def find(p):
        ...
        words = p.split()
        return any(sub in words for sub in subject)
...
def accuracy(typed, source):
    ...
    if len(typed_words) > len(source_words):
        wrong_count += len(typed_words) - len(source_words)
    for i in range(min(len(typed_words), len(source_words))):
        if typed_words[i] != source_words[i]:
            wrong_count += 1

    return (1 - wrong_count / len(typed_words)) * 100
    # END PROBLEM 3
...
def furry_fixes(typed, source, limit):
    ...
    def furry(i, err):
        if i >= min_len:
            return max_len - min_len + err
        else:
            if typed[i] != source[i]:
                err += 1
                if err > limit:
                    return err
            return furry(i+1, err)
        
    return min(furry(0,0), limit + 1)
    # END PROBLEM 6
...
def minimum_mewtations(typed, source, limit):
    ...
    3
    """
    if limit < abs(len(typed) - len(source)):
        return limit + 1
...
def time_per_word(words, timestamps_per_player):
    ...
    for i in range(len(timestamps_per_player)):
        ...
        for j in range(len(words)):
            t_i.append(timestamps_per_player[i][j+1] - timestamps_per_player[i][j])
        t.append(t_i)
    return words, t
    # END PROBLEM 9
...
def fastest_words(match_object):
    ...
    # BEGIN PROBLEM 11
    "*** YOUR CODE HERE ***"
    w = [[] for _ in player_indices]
    for i in word_indices:
        min_index = 0
        for j in player_indices[1:]:
            if get_time(match_object, j, i) < get_time(match_object, min_index, i):
                min_index = j
        w[min_index].append(get_word(match_object, i))   
    return w        
    # END PROBLEM 11
