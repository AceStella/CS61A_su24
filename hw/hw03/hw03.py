...
def balanced(m):
    ...
    """
    "*** YOUR CODE HERE ***"
    torque0 = length(left(m)) *total_mass(end(left(m)))
    torque1 = length(right(m)) *total_mass(end(right(m)))
    return torque0 == torque1 and (is_planet(end(left(m))) or balanced(end(left(m)))) and (is_planet(end(right(m))) or balanced(end(right(m))))
