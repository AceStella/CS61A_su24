...
class Player:
    ...
    def debate(self, other):
        "*** YOUR CODE HERE ***"
        if max(0.1, self.popularity / (self.popularity + other.popularity)) >= self.random_func():
            self.popularity += 50
        else:
...
def store_digits(n):
    ...
    def range_link(l):
        if len(l) == 1:
            ...
            return Link(l[0], range_link(l[1:]))
    return range_link(l[::-1])
...
def deep_map_mut(func, lnk):
    ...
    while isinstance(lnk, Link):
        if isinstance(lnk.first, Link):
            # If the first element is a linked list, apply deep_map_mut recursively
            deep_map_mut(func, lnk.first)
        else:
            # Apply func to the first element
